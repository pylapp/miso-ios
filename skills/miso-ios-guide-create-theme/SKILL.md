...
### 2.5 Real example — `FoxyRoughTheme`

`FoxyRoughTheme` (module `MISOThemesMISOFoxyRough`, in `MISO/Core/MISO/Themes/FoxyRough/`) is a real, minimal Strategy A theme shipped in this repository. It subclasses `BlueCoatTheme` and overrides only three providers:

- `FoxyRoughColorTokensProvider` (subclasses `BlueCoatThemeColorSemanticTokensProvider`) — cold grey backgrounds with blood-red brand accents.
- `FoxyRoughFontSemanticTokensProvider` (subclasses `BlueCoatThemeFontSemanticTokensProvider`) — the "Winky Rough" custom font family (see §6 for `.ttf` registration).
- `FoxyRoughButtonTokensProvider` (subclasses `BlueCoatThemeButtonComponentTokensProvider`) — brand-specific button colors, border widths and radius.

All other providers (borders, sizes, spaces, grids, and the 26 other component providers) are left untouched and fall back to `BlueCoatTheme`'s defaults. Its own `Tuning` is built inline (rounded buttons, text inputs, alert messages, progress indicators and list items) and passed to `super.init(tuning:)`.

Use this as the reference for a **partial override** theme: read `MISO/Core/MISO/Themes/FoxyRough/Sources/` for the full source, and `MISO/Core/MISO/Themes/FoxyRough/Tests/` for its test suite (see §12 below).

> **Important:** when calling `super.init(...)`, always pass `name:` explicitly with your theme's own name. If omitted, `theme.name` silently falls back to the base theme's name (e.g. `"BlueCoat"`), which breaks debugging/analytics and any test asserting `theme.name`.
...
- [ ] `name:` is passed explicitly to `super.init(...)` with your theme's own name (otherwise `theme.name` silently falls back to the base theme's name)
- [ ] Unit tests exist for the theme and its overridden providers — see §12 below

---

## 12. Testing your theme

Every theme in this repository (e.g. `BlueCoatTheme`, `FoxyRoughTheme`) ships with a **unit test target** that checks the *types* and the *values* of its providers. New themes should follow the same convention.

### 12.1 Test framework and conventions

- Use **Swift Testing** (`import Testing`, `@Test`, `#expect`, `#require`), **not XCTest**.
- One `struct` per test suite, with an `init()` that builds a fresh instance of the theme or provider under test.
- Two kinds of tests:
  - **Type tests** — assert that a theme/provider exposes the expected **concrete type** for each (overridden or inherited) provider: `#expect(theme.colors is YourThemeColorProvider)`.
  - **Value/relationship tests** — assert **values** (exact colors, exact raw token references) and **relationships** between tokens (`small < medium < large`, lighter/darker, hexadecimal format via regex). Prefer relationships over hard-coded literals for anything that could be regenerated by an external tool (raw tokens).
- Reuse the shared `TestsUtils` module (target `TestsUtils`, product of `MISO/Foundations/OUDS/TestsUtils`) for common assertions:
  - `TestsUtils.assertColorLighterThan(_:_:)` / `assertColorDarkerThan(_:_:)` — compare two hexadecimal color strings.
  - `TestsUtils.assertMatches(_:regexp:)` — check a string against a regular expression (e.g. `^#[0-9A-Fa-f]{8}$` for color format).
  - `TestsUtils.assertBetween(min:_:max:)`, `assertMultipleOf(_:factor:)` — numeric checks.

### 12.2 Directory layout and `Package.swift` wiring

Mirror the `BlueCoat`/`FoxyRough` layout, e.g. for a theme named `YourTheme`:

```
MISO/Core/MISO/Themes/YourTheme/
├── Sources/                         ← theme implementation (see §1-§10)
└── Tests/
    ├── YourThemeTests.swift         ← top-level: theme name, tuning, provider types
    ├── Providers/
    │   ├── ComponentTokens/         ← one file per overridden component provider
    │   └── SemanticTokens/          ← one file per overridden semantic provider
    └── Values/
        └── RawTokens/               ← format + relationship tests for custom raw color/font tokens
```

Add a matching test target in `Package.swift`, right after your theme's `.target(...)` declaration:

```swift
.testTarget(
    name: "MISOThemesMISOYourTheme-Tests",
    dependencies: ["TestsUtils", "MISOThemesMISOBlueCoat", "MISOThemesMISOYourTheme"],
    path: "MISO/Core/MISO/Themes/YourTheme/Tests"),
```

### 12.3 What to test, at minimum

- **`YourThemeTests`** (top-level): `theme.name == "YourTheme"`, `theme.fontFamily`, `theme.tuning` field values, and `is` type checks for every provider you overrode **and** a few you didn't (to confirm they still fall back to the base theme's provider).
- **Per overridden semantic/component provider**: one test file asserting the `light`/`dark` values (or raw token references) of each overridden property — see `FoxyRoughColorTokensProviderTests`, `FoxyRoughFontTokensProviderTests`, `FoxyRoughButtonTokensProviderTests` in `MISO/Core/MISO/Themes/FoxyRough/Tests/Providers/` for a concrete example.
- **Custom raw tokens** (if you define your own color/font enum, e.g. `FoxyRoughColors`): format regex + lighter/darker relationships + "all colors are different" — see `FoxyRoughColorsTests` in `MISO/Core/MISO/Themes/FoxyRough/Tests/Values/RawTokens/`.

### 12.4 Running the tests

```bash
swift build --target MISOThemesMISOYourTheme-Tests
swift test --filter YourTheme
```
