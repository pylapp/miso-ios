...
// MARK: - MISO Low Power Mode Observer

/// An `ObservableObject` providing through a published state a flag saying if the device  is in low pover mode or not.
/// This object is exposed as *environment object* through the `MISOThemeableView`, and can be used in any view
/// inside it to apply a specific behavior when the device is in low power mode.
///
/// ```swift
/// // Inside a view with the MISOThemeableView as root view
/// @EnvironmentObject var lowPowerModeObserver: MISOLowPowerModeObserver
///
