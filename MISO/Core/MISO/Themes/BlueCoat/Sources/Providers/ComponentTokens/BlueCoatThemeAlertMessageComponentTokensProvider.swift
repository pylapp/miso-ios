...
/// - Since: 1.0.0
public typealias DefaultAlertMessageComponentTokensProvider = BlueCoatThemeAlertMessageComponentTokensProvider

/// A class which wraps all **component  tokens of alert** for *alert* objects like `MISOAlertMessage`.
/// Contains also references to semantic tokens providers so as to be able to use them to define the component tokens.
/// This provider should be integrated as a `AllAlertMessageComponentTokensProvider` implementation inside `MISOTheme` so as to provide
...
/// It implements also the protocol `AlertMessageComponentTokens` so as to expose the component tokens for *alertMessage* through any `MISOTheme`.
...
///
/// - Since: 1.0.0
open class BlueCoatThemeAlertMessageComponentTokensProvider: AllAlertMessageComponentTokensProvider {
    ...
    {
        ML.debug("Init of BlueCoatThemeAlertMessageComponentTokensProvider")
        ...
        #if DEBUG
        Self.instanceCount++
        checkInstances(count: Self.instanceCount, for: "BlueCoatThemeAlertMessageComponentTokensProvider")
        #endif
    }
    ...
    // the implementation of AlertMessageMessageComponentTokens is not here but in Core/MISO/Themes/BlueCoat/Values/ComponentTokens/BlueCoatTheme+AlertMessageComponentTokens.swift
    // This declaration of BlueCoatThemeAlertMessageComponentTokensProvider is here also to allow to write documentation.
}
