...
/// - Since: 1.0.0
public typealias DefaultLinkComponentTokensProvider = BlueCoatThemeLinkComponentTokensProvider

/// A class which wraps all **component  tokens of links** for *link* objects like `MISOLink`.
/// Contains also references to semantic tokens providers so as to be able to use them to define the component tokens.
/// This provider should be integrated as a `AllLinkComponentTokensProvider` implementation inside `MISOTheme` so as to provide
...
/// It implements also the protocol `LinkComponentTokens` so as to expose the component tokens for *links* through any `MISOTheme`.
...
///
/// - Since: 1.0.0
open class BlueCoatThemeLinkComponentTokensProvider: AllLinkComponentTokensProvider {
    ...
    ///    - dimensions: Provider for dimensions semantic tokens. If nil, a default one will be used (``BlueCoatThemeDimensionSemanticTokensProvider``)
    public init(sizes: AllSizeSemanticTokensProvider? = nil,
                colors: AllColorSemanticTokensProvider? = nil,
                spaces: AllSpaceSemanticTokensProvider? = nil,
                dimensions: AllDimensionSemanticTokensProvider? = nil)
    {
        ML.debug("Init of BlueCoatThemeLinkComponentTokensProvider")
        ...
        #if DEBUG
        Self.instanceCount++
        checkInstances(count: Self.instanceCount, for: "BlueCoatThemeLinkComponentTokensProvider")
        #endif
    }
    ...
    // the implementation of LinkComponentTokens is not here but in Core/MISO/Themes/BlueCoat/Values/ComponentTokens/BlueCoatTheme+LinkComponentTokens.swift
    // This declaration of BlueCoatThemeLinkComponentTokensProvider is here also to allow to write documentation.
}
