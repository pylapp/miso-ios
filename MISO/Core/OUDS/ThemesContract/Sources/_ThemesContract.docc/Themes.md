...
Then, a white label theme may just inherit from this class and override the tokens it needs. A concrete example is the `BlueCoatTheme`, providing in its module its own colors and overriding some tokens with the suitable values.
...
```swift
// Add themeable view to your root view to use the BlueCoatTheme
...
By default, the `BlueCoatTheme` is not tuned.
...
You will have to create a _Swift class_ which will inherit from an existing theme like `BlueCoatTheme` (if you need its assets and resources) or from `MISOTheme` directly.
You can see `BlueCoatTheme` as more specified and less abstract as `MISOTheme` which is the base of all themes.
...
You may want to define your own theme, thus you can override an existing theme like `BlueCoatTheme` with your own class or just override the providers.
For clarity reasons maybe you should define your own class inheriting from `BlueCoatTheme`, or, more difficult, from `MISOTheme` but we do not recommand that.

You must consider the tokens provider you need (to inherit from for overriding, or to use as is):
- spaces tokens are `BlueCoatThemeSpaceSemanticTokensProvider`
- sizes tokens are in `BlueCoatThemeSizeSemanticTokensProvider`
- colors tokens are all defined in `BlueCoatThemeColorSemanticTokensProvider`
- borders tokens are in `BlueCoatThemeBorderSemanticTokensProvider`
- elevations tokens are in `BlueCoatThemeElevationSemanticTokensProvider`
- opacity tokens are in `BlueCoatThemeOpacitySemanticTokensProvider`
- grid tokens are in `BlueCoatThemeGridSemanticTokensProvider`
- font tokens are in `BlueCoatThemeFontSemanticTokensProvider`
...
class YourAppThemeSpaceTokensProvider: BlueCoatThemeSpaceSemanticTokensProvider {
...
class YourAppThemeSizeTokensProvider: BlueCoatThemeSizeSemanticTokensProvider {
...
class YourAppThemeColorTokensProvider: BlueCoatThemeColorSemanticTokensProvider {
    override var bgSecondary: MultipleColorSemanticToken {
        MultipleColorSemanticToken(light: ColorRawTokens.colorDecorativeAmber500, dark: BlueCoatBrandColorRawTokens.royalBlue300)
    }
...
class YourAppThemeBorderTokensProvider: BlueCoatThemeBorderSemanticTokensProvider {
...
class YourAppThemeElevationTokensProvider: BlueCoatThemeElevationSemanticTokensProvider {
...
class YourAppThemeOpacityTokensProvider: BlueCoatThemeOpacitySemanticTokensProvider {
...
class YourAppThemeGridTokensProvider: BlueCoatThemeGridSemanticTokensProvider {
...
class YourAppThemeFontTokensProvider: BlueCoatThemeFontSemanticTokensProvider {
...
```swift
import MISOThemesMISOBlueCoat // To get BlueCoatTheme

// Define your theme
class YourAppTheme: BlueCoatTheme {
