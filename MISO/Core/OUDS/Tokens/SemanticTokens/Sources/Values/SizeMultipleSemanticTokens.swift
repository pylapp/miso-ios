...
/// A major part the "real" size semantic tokens are declared in ``SizeSemanticTokens`` protocol and defined inside `MISOTheme` (to be overridable then by subthemes), and othershere.
...
///         // declared (in SizeSemanticTokens.swift) and defined (in MISOTheme+SizeSemanticTokens.swift) by the tokenator
...
///         // declared (in SizeMultipleSemanticTokens.swift) and defined (in MISOTheme+SizeMultipleSemanticTokens.swift) manualy
