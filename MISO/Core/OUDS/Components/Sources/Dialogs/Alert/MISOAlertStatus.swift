...
@frozen public enum MISOAlertStatus {
    ...
    ///
    /// - Parameter image: Optional `MISOImage`  to be displayed in the alert. Pass `nil` if no icon is needed.
    case neutral(image: MISOImage? = nil)
    ...
    ///
    /// - Parameter image: Optional `MISOImage`  to be displayed in the alert. Pass `nil` if no icon is needed.
    case accent(image: MISOImage? = nil)
