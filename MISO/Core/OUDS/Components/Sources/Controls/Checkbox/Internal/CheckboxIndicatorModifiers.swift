...
private struct CheckboxIndicatorForegroundModifier: ViewModifier {
    ...
    private var readOnlyColor: MultipleColorSemanticToken {
        guard !isError else {
            ML.fatal("An MISO Checkbox with a read only state and an error situation has been detected, which is not allowed."
    ...
    private var disabledColor: MultipleColorSemanticToken {
        guard !isError else {
            ML.fatal("An MISO Checkbox with a disabled state and an error situation has been detected, which is not allowed."
                + " Only non-error situation are allowed to have a disabled state.")
        }
...
private struct CheckboxIndicatorBackgroundModifier: ViewModifier {
    ...
    private var hoverColor: Color {
        theme.listItem.colorBgHover.color(for: colorScheme)
    }

    private var pressedColor: Color {
        theme.listItem.colorBgPressed.color(for: colorScheme)
    }
