...
The ``MISOTabBar`` provides a native SwiftUI `TabView` with use of MISO effects and styles.
...
The `MISOTabView` and `MISOLiquidGlassTabView` use the native SwiftUI `Tab` API with MISO styles.
...
The toolbars components provide top and bottom navigation bars. They rely on SwiftUI toolbars and expose MISO styles.
...
.misoNavigationBarAppearance() // required — apply on the NavigationStack, not on the child view
