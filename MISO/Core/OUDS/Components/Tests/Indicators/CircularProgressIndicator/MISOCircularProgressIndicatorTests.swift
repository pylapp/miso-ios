...
/// Tests on the public API and internal configuration of ``MISOCircularProgressIndicator``.
struct MISOCircularProgressIndicatorTests {

    // MARK: - View constants

    @Test func `default size constant must be 48 points`() {
        #expect(MISOCircularProgressIndicator.defaultSize == 48.0)
    }
    ...
    func `determinate configuration must expose the provided progress and defaults for other fields`() {
        // Mirrors the defaults applied by MISOCircularProgressIndicator.init(progress:status:track:gapSize:).
        let configuration = CircularProgressIndicatorConfiguration(progress: 0.5,
                                                                   status: .neutral,
                                                                   track: true,
                                                                   gapSize: .default,
                                                                   size: MISOCircularProgressIndicator.defaultSize)
        ...
        #expect(configuration.size == MISOCircularProgressIndicator.defaultSize)
    ...
    func `indeterminate configuration must have nil progress and defaults for other fields`() {
        // Mirrors the defaults applied by MISOCircularProgressIndicator.init(status:track:gapSize:).
