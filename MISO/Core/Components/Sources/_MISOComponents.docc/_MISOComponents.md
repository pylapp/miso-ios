...
With MISO, typography depends to the class size, i.e. whether or not the application is in _compact mode_ or in _regular mode_, and is defined with a [`MultipleFontCompositeSemanticToken`](https://ios.unified-design-system.orange.com/documentation/misotokenssemantic/multiplefontcompositesemantictoken) defined in the [`FontSemanticTokens`](https://ios.unified-design-system.orange.com/documentation/misotokenssemantic/fontsemantictokens/).
...
These view modifiers are available for any `View` object, [you can get the curated list in the documentation](https://ios.unified-design-system.orange.com/documentation/misocomponents/swiftuicore/view).
...
Some helpers are available in the MISO API to avoid to use the `color(for:)` method.
...
If you use for example a theme configured with a system font, you may for sure use the *Helvetica Neue* font family.
...
func localizedHelveticaFont() -> String {
    return (MYOUtils.isArabicLanguageInUse() ? "Helvetica Neue Arabic" : "Helvetica Neue")
}

/// Instanciate your theme using the font family.
/// Thus when the user will change the app language the app will be restarted, repainted and the theme updated
let localizedTheme: MISOTheme = WireframeTheme(fontFamily: localizedHelveticaFont())
```

> Caution: For legal reasons MISO does not provide the Helvetica Neue Arabic nore Helvetica Neue assets. You will have to get them and register the fonts files in your app

> Note: For cyrillic alphabet *Helvetica Neue* has no variant, you can use *Arial* instead
...
Then, for each weight variant you need, declare the PostScript name so MISO can resolve the correct identifier from its font tokens:
